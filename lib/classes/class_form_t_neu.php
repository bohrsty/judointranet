	...
	protected function read_daten() {
		...
		for($i=0;$i<count($explode);$i++) {
			...
			switch($typ) {
				...
				case 'NBselect':
					$daten[] = new NBselect($name,'class="termin-select"',1,$this->read_optionen($name),$erforderlich);
				break;
				
				case 'NBinput':
					$daten[] = new NBinput($name,'text','class="termin-input"',$erforderlich,$datentyp);
				break;
			}
		}
